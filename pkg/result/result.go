...
// ScenarioResults each scenario could have multiple result
...
// CheckResults will check to see if there are result with a specific Profile like TCP_STREAM
// returns true if there are result with provided string
...
// CheckHostResults will check to see if there are hostNet result
// returns true if there are result with hostNetwork
...
// Abstracts out the common code for result
...
func ShowStreamResult(s ScenarioResults) {
	if checkResults(s, "STREAM") {
		logging.Debug("Rendering Stream result")
		renderResults(s, "STREAM")
	}
}

// ShowRRResult will display the RR performance result
// Currently showing the Avg Value.
func ShowRRResult(s ScenarioResults) {
	if checkResults(s, "RR") {
		logging.Debug("Rendering RR result")
		renderResults(s, "RR")
	}
}
